...
class SimpleEpisode {
	...
	public function __construct($load) {
		global $Wcms;
		$this->dbPath = $Wcms->dataPath . '/simpleepisode.json';
		if ($load) {
			$this->Wcms =&$Wcms;
		}
	}
			...
			file_put_contents($this->dbPath, json_encode([
				...
				'posts' => [
					'hello-world' => [
						...
						'date' => time(),
						'body' => "This is the full episode post content. Here's some more example text. Consectetur adipisicing elit. Quidem nesciunt voluptas tempore vero, porro reprehenderit cum provident eum sapiente voluptate veritatis, iure libero, fugiat iste soluta repellendus aliquid impedit alias.",
					],
				],
	...
	public function startListener(array $args): array {
		...
		if ($this->active) {
			// Remove page doesn't exist notice on episode pages
		...
		$args[0] .= "<link rel='stylesheet' href='{$this->Wcms->url('plugins/simple-episode/css/episode.css')}'>";
	...
	public function jsListener(array $args): array {
		...
		$args[0] .= "<script src='{$this->Wcms->url('plugins/simple-episode/js/episode.js')}'></script>";

		return $args;
	}

	public function menuListener(array $args): array {
		// Add episode menu item
		$extra = $this->active ? 'active ' : '';

		$args[0] .= <<<HTML
        <li class="{$extra}nav-item">
            <a class="nav-link" href="{$this->Wcms->url($this->slug)}">Episode</a>
        </li>
HTML;
			...
			switch ($this->path[0]) {
				case '':
					...
					if ($this->Wcms->loggedIn) {
						$args[0] = "<div class='text-right'><a href='#' class='btn btn-light' onclick='episode.new(); return false;'><span class='glyphicon glyphicon-plus-sign'></span> Create new post</a></div>";
					}
				...
				default:
					if (isset($this->db->posts->{$this->path})) {
						...
						if ($this->Wcms->loggedIn) {
							$args[0] = <<<HTML
                            <div class="post">
                                <div data-target="episode" style='margin-top:0;' id="title" class="title editText editable">{$post->title}</div>
                                <p class="meta">{$date} &nbsp; &bull; &nbsp; <a href='{$this->Wcms->url('plugins/simple-episode/delete.php')}?page={$this->path}&token={$this->Wcms->getToken()}' onclick='return confirm(\"Are you sure you want to delete this post?\")'>Delete</a></p>
                                <hr>
                                <div data-target="episode" id="description" class='meta editText editable'>{$post->description}</div>
                                <hr>
                                <div data-target="episode" id="body" class="body editText editable">{$post->body}</div>
                            </div>
HTML;
						} else {
							$args[0] = <<<HTML
...
HTML;
                        ...
                        <div class="text-left">
                            <br /><br />
                            <a href="../$this->slug" class="btn btn-sm btn-light"><span class="glyphicon glyphicon-chevron-left small"></span> Back to all episode posts</a>
                        </div>
HTML;
					} else {
